...
# Цвета (отключаем, если вывод не в TTY или установлен NO_COLOR)
...
check_states() {
    local states=("Хаос" "Тупик" "Поворот")
...
check_practices() {
    local practices=("Слот саморазвития" "Мимолётные заметки" "Трекер-помидоро" "Рабочий продукт" "ИИ-ассистент" "Метод стоп-момента")
    local found=0
